# ===========================================================
#  SmartCar - Servidor Principal (OPTIMIZADO MEMORIA)
# ===========================================================
import sys
import os
...
# NOTA: procesar_validacion_acceso ahora espera un DICCIONARIO, no un string
...
# CORS PERMISIVO (Vital para Vercel)
...
def token_requerido(f):
    ...
    def decorador(*args, **kwargs):
        ...
        except: return jsonify({"error": "Token inválido"}), 401
        return f(*args, **kwargs)
    return decorador

# --- RUTAS GENERALES ---
@app.route("/")
def index(): return "SmartCar API Running"
...
def login():
    try:
        ...
        token = jwt.encode({
            ...
            "id_audit": user["id_audit"], "exp": datetime.utcnow() + timedelta(hours=8)
...
# --- RUTA CRÍTICA DE OCR (MODIFICADA) ---
@app.route("/api/accesos/validar", methods=["POST"])
def validar_acceso_ocr():
    try:
        # 1. Usamos get_json() para asegurar que Flask parsee el cuerpo correctamente
        data_dict = request.get_json(force=True)
        if not data_dict:
            return jsonify({"error": "No se recibieron datos JSON"}), 400
        
        # 2. Pasamos el diccionario directamente al controlador
        respuesta, status = procesar_validacion_acceso(data_dict, vigilante_id=1)
        
        # 3. FORZAR LIMPIEZA DE MEMORIA (Vital para plan gratis)
        gc.collect()
        
        return jsonify(respuesta), status
    except Exception as e:
        print(f"❌ Error en ruta validar: {e}")
...
def exportar_excel():
    try:
        ...
        wb = Workbook()
        # ... (Lógica simplificada para no alargar, usa la de admin_model)
        ws = wb.active; ws.append(["Reporte", f"{fi} - {ff}"])
...
def exportar_pdf():
    ...
    try:
        ...
        reporte = obtener_data_reporte_completo(fi, ff)
        buffer = BytesIO(); p = canvas.Canvas(buffer, pagesize=letter)
        p.drawString(100, 750, f"Reporte SmartCar: {fi} a {ff}")
...
def delete_alerta(id_a):
    if eliminar_alerta_controller(id_a, request.usuario_actual, request.get_json().get('accion_resolucion')):
